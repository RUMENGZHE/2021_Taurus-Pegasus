...
hisignalling_error_type hisignalling_msg_receive(hi_u8 *buf, hi_u32 len)
{
    ...
    if (len > HISGNALLING_MSG_CRC32_LEN) {
       ...
       switch (buf[HISIGNALLING_MSG_PAYLOAD_INDEX]){
        case FIST:
            HISIGNALLING_LOG_INFO("FIST\n");
            hi_gpio_set_ouput_val(HI_GPIO_IDX_9,HI_GPIO_VALUE1);
        break;
        case PALM:
            HISIGNALLING_LOG_INFO("PLAM\n");
            hi_gpio_set_ouput_val(HI_GPIO_IDX_9,HI_GPIO_VALUE0);
        break;
        case X_BIG:
            HISIGNALLING_LOG_INFO("X_BIG\n");
        break;
        case X_LIT:
            HISIGNALLING_LOG_INFO("X_LIT\n");
        break;
        case Y_BIG:
            HISIGNALLING_LOG_INFO("Y_BIG\n");
        break;
        case Y_LIT:
            HISIGNALLING_LOG_INFO("Y_LIT\n");
        break;
        case OTHERS:
            HISIGNALLING_LOG_INFO("OTHERS\n");
        break;
    }
    /*输出回显收到的数据*/ 
