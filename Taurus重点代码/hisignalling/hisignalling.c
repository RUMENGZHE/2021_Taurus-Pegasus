...
void usbUartSendRead(int fd, HandClassification refuseType)
{
	...
	unsigned char write_buffer5[4] = {0,2,0,4};
	unsigned char write_buffer6[4] = {0,2,0,5};
	unsigned char write_buffer7[4] = {0,2,0,6};
	unsigned char write_buffer8[4] = {0,2,0,7};
    ...
    switch (refuseType) {
        ...
        case OTHERS:
			...
			break;   		
		case X_BIG:
            hisignalling_msg_send(fd, write_buffer5, sizeof(write_buffer5)/sizeof(write_buffer5[0]));
            printf("X_BIG\r\n");
			break;
        case X_LIT:
            hisignalling_msg_send(fd, write_buffer6, sizeof(write_buffer6)/sizeof(write_buffer6[0]));
            printf("X_LIT\r\n");
			break;
        case Y_BIG:
            hisignalling_msg_send(fd, write_buffer7, sizeof(write_buffer7)/sizeof(write_buffer7[0]));
            printf("unqualified\r\n");
			break; 
		case Y_LIT:
            hisignalling_msg_send(fd, write_buffer8, sizeof(write_buffer8)/sizeof(write_buffer8[0]));
            printf("Y_LIT\r\n");
			break; 	
    }
